...
## 12.1 改动前必读的代码红线（`AGENTS.md`）

改任何 GsCore 代码前必读仓库根目录 [`AGENTS.md`](../../../../AGENTS.md)。绝对红线：
...
[`AI_AGENT_LIFECYCLE_SEQUENCE.md`](../../../../docs/AI_AGENT_LIFECYCLE_SEQUENCE.md) §10.4–§10.6。
...
已修复（明细见 [`docs/AI_CORE_CHANGE_REVIEW_20260712.md`](../../../../docs/AI_CORE_CHANGE_REVIEW_20260712.md)）。
后续改这些模块必须守住的点：
...
## 12.22h 套件化 / 关系温度改造踩到的四个坑（2026-08-15 实机基准复盘）

三线改造（Hook 总线 + 18 套件 / 关系温度 / 认知层）在**单元测试全绿、pyright 与 ruff
全清**的状态下，实机群聊基准从 87.5% 掉到 58.3%。四个根因全是「集成后才成立」的错误。
完整表格见 `docs/AI_CORE_THREE_LINE_REFACTOR_20260815.md` §10.4，这里只留可复用的规律。

**🔴 一、「注册表为空」有两种含义，混淆即事故**

`_kernel_owns_tool_assembly()` 用「`tool_assembly` 槽有没有占用者」判断内核该不该跑五层
工具装配。但占用表是 `load_enabled_kits` 填的，而它当时排在 `_INIT_STEPS` **末尾**。
于是「套件还没装好」被读成「用户把槽拆了」，整个启动窗口内所有请求退化成**零工具**
（`find_tools` 一并消失）。Meme 的一次性向量迁移占用 337 秒，那 337 秒里基准 24 例
`tools~0.0`，通过率 37.5%。

> **判据能取配置就不要取运行期状态。** 配置在进程启动时就已就绪，不参与启动竞态。
> 若必须看运行期注册表，先区分 *用户显式关了*（配置说 `off`）与 *还没装好*
> （注册表空），并默认站在 **fail-open** 一侧。

**🔴 二、一个子系统只能有一个初始化主**

能力搬进套件时，五个套件的 `init_step` 与 `_INIT_STEPS` 的旧条目**同时**存在：
memory / statistics / planning / meme / favor_decay 每次启动都初始化两遍。不报错、
不告警，只是 Meme 的 337 秒迁移跑了两次并强制重建了两次 Qdrant collection。

> 搬迁 = 新增 + **删除旧的那处**。只做前一半时系统仍然「工作」，所以没人会发现。
> 现在由 `tests/test_agent_kits_slots.py::test_kit_init_steps_do_not_duplicate_core_init_steps`
> 比对两侧 `await` 到的调用名来锁死。

**🔴 三、每轮注入的文本要和人设卡叠加起来读，效果是相乘不是相加**
...
> 两条规矩：
> 1. **调语气的旋钮，每一档都要自带履约地板**（「该查该办照做」）。口气是旋钮，
>    履约是地板——`_ZONE_VOICE` 的四个低档现在都带这句，有测试锁。
> 2. **没有依据就不要拧旋钮**。`scored=False` 是「没打过分」，不是「陌生」；
>    此时 `RelationshipView.voice` 必须返回空串，套件不注入。
>
> 自查方式：把本轮真实注入的 user 侧文本和人设卡拼在一起通读一遍，问「这段话
> 允许我不干活吗」。别只 review 自己新加的那一段。

**🔴 四、`create_by` 的授权集合必须处处一致，否则评测测的不是生产**

`"TEST"`（本地评测端点）在 `_INTERACTIVE_CREATE_BY` 里被当作交互主人格，却不在
`allow_outbound` 的授权集合 `("Chat", "Agent")` 里。后果：评测里的主人格调
`send_message_by_ai` 交付委派出的图时，收到「当前为能力代理/子 Agent，禁止直发」，
据此认为自己没有出站权，直接吐 `<SILENCE>`——`data_rendering` 整域 0/4。

> 新增或修改任何按 `create_by` 分流的集合时，把**所有**相关集合一起列出来对齐：
> `_INTERACTIVE_CREATE_BY` / `_AGENTIC_CREATE_BY` / `_MAIN_PERSONA_CREATE_BY` /
> `allow_outbound`。一个值在某一处算主人格、在另一处算子代理，就是 bug。

**🔴 五、方法论：判分器与被测对象共用一个 core 时，超时会伪装成能力缺陷**

`eval/agent` 的 bot-judge 打的是同一个运行中的 core。批量跑完后 provider 仍在排队，
判分请求单次 90s 超时被静默记成 FAIL——实测 **6 例假失败**（同一 prompt 事后复核
3/3 PASS）。同理，基准若与一次性数据迁移并行，延迟类断言会集体撞上限。

> 报出的失败先分三类再动手改代码：**真失败 / 判分器抖动 / 环境争用**。
> 分类手段：① 失败原因里带上实际调用的工具名（`must_call_any` 现在会报 `called=[...]`）；
> ② 用同一 prompt 单独复跑判分器；③ 单请求隔离计时。
> 本次三类各占一部分——先分类，避免为了「修」判分器抖动而把正确逻辑改坏。
...
12. 套件 / 启动顺序（§12.22h）：新套件的 `init_step` 没有和 `_INIT_STEPS` 撞同一个
    bring-up；以注册表为判据的门区分了「用户关了」与「还没装」并 fail-open；
    按 `create_by` 分流的四个集合口径一致。
13. 每轮注入的 prompt 片段（§12.22h）：和人设卡拼起来通读过，没有叠加出「可以不办事」；
